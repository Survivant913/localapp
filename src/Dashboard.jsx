...
export default function Dashboard({ data, updateData, setView }) {
    ...
    // On garde uniquement les non terminés pour le dashboard
    ...
    return (
        <div className="space-y-8 fade-in p-4 pb-20 max-w-7xl mx-auto">
            {focusedProject && (
                <FocusProjectModal 
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex-1 bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-gray-200 dark:border-slate-700 flex flex-col justify-between">
                    <div className="flex justify-between items-start mb-6">
                        <div>
                            <h2 className="text-3xl font-bold text-gray-800 dark:text-white mb-2">{labels.userName || 'Bonjour,'}</h2>
                            <p className="text-gray-500 dark:text-gray-400 text-lg capitalize">{todayDate}</p>
                        </div>
                        <select 
                            value={dashboardFilter} 
                            onChange={(e) => setDashboardFilter(e.target.value)} 
                            className="px-4 py-2 bg-gray-50 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-xl text-sm font-bold outline-none text-slate-800 dark:text-white"
                        >
                    ...
                    <div className="flex gap-4 mt-auto">
                        <button onClick={() => setView('budget')} className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl text-base font-bold shadow-lg shadow-blue-900/20 transition-transform hover:scale-[1.02]"><Plus size={20}/> Dépense</button>
                        <button onClick={() => setView('todo')} className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-gray-800 dark:text-white rounded-2xl text-base font-bold transition-transform hover:scale-[1.02]"><CheckSquare size={20} className="text-orange-500"/> Tâche</button>
                        <button onClick={() => setView('notes')} className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-gray-800 dark:text-white rounded-2xl text-base font-bold transition-transform hover:scale-[1.02]"><StickyNote size={20} className="text-yellow-500"/> Note</button>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {/* CARTE SOLDE (Theme Aware) */}
                <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-lg border border-gray-200 dark:border-slate-700 relative overflow-hidden flex flex-col justify-between min-h-[260px]">
                    ...
                    <div className="relative z-10 mt-4 mb-4">
                        <h3 className="text-4xl font-extrabold text-gray-900 dark:text-white tracking-tight mb-1">{renderAmount(currentBalance)}</h3>
                ...
                <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-lg border border-gray-200 dark:border-slate-700 flex flex-col">
                    ...
                    <div className="flex-1 flex flex-col gap-3">
                        {upcomingList.length === 0 ? (
                            <div className="flex-1 flex items-center justify-center text-gray-400 dark:text-slate-500 text-sm">Rien à signaler</div>
                        ) : (
                            upcomingList.map((e, idx) => (
                                <div key={idx} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-800/50 rounded-xl border border-gray-100 dark:border-slate-700">
                                    <div className="flex items-center gap-4">
                                        <div className="p-2 bg-white dark:bg-slate-700 rounded-lg text-gray-500 dark:text-slate-300 shadow-sm">{e.type === 'scheduled' ? <Calendar size={18}/> : <TrendingUp size={18}/>}</div>
                                        <div>
                                            <p className="font-bold text-gray-800 dark:text-white text-sm">{e.data.description}</p>
                                            <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-slate-400">
                                                <span>{e.date.toLocaleDateString()}</span>
                                                {e.type === 'recurring' && <span className="flex items-center gap-1"><TrendingUp size={10}/> Récurrent</span>}
                                            </div>
                                        </div>
                                    </div>
                                    <span className={`font-bold text-sm ${e.data.type === 'income' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{renderAmount(parseFloat(e.data.amount), true)}</span>
                                </div>
                            ))
                        )}
                    </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-gray-200 dark:border-slate-700 shadow-sm">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={handleAutoFocus} className="flex items-center gap-2 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-bold transition-colors shadow-lg shadow-indigo-900/20"><Target size={14}/> FOCUS AUTO</button>
                        </div>
                        
                        <div className="space-y-4">
                            {activeProjects.length === 0 ? (
                                <p className="text-gray-400 dark:text-slate-500 text-sm italic">Aucun projet en cours.</p>
                            ) : (
                                activeProjects.map(p => {
                                    ...
                                    return (
                                        <div key={p.id} className="bg-gray-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-gray-100 dark:border-slate-700 hover:border-blue-200 dark:hover:border-slate-600 transition-colors cursor-pointer" onClick={() => setView('projects')}>
                                            <div className="flex items-center gap-5">
                                                {/* CERCLE GAUCHE */}
                                                <div className="relative inline-flex items-center justify-center w-14 h-14 shrink-0">
                                                    <svg className="w-full h-full transform -rotate-90">
                                                        <circle cx="28" cy="28" r="24" stroke="currentColor" strokeWidth="4" fill="none" className="text-gray-200 dark:text-slate-700" />
                                                        <circle cx="28" cy="28" r="24" stroke="currentColor" strokeWidth="4" fill="none" strokeDasharray={24 * 2 * Math.PI} strokeDashoffset={24 * 2 * Math.PI * (1 - globalScore / 100)} className={`transition-all duration-1000 ${globalScore >= 100 ? 'text-green-500' : 'text-blue-500'}`} strokeLinecap="round" />
                                                    </svg>
                                                    <span className="absolute text-[10px] font-bold text-gray-700 dark:text-slate-200">{Math.round(globalScore)}%</span>
                                                </div>
                                                ...
                                                <div className="flex-1 min-w-0 space-y-2">
                                                    <div className="flex justify-between items-start">
                                                        <div>
                                                            <h4 className="font-bold text-gray-800 dark:text-white text-base truncate">{p.title}</h4>
                                                            <div className="flex items-center gap-2 mt-0.5">
                                                                <span className={`w-2 h-2 rounded-full ${p.status === 'in_progress' ? 'bg-green-500' : 'bg-blue-500'}`}></span>
                                                                <p className="text-xs text-gray-500 dark:text-slate-400 truncate">{p.description || "Pas de description"}</p>
                                                            </div>
                                                        </div>
                                                        {p.status === 'in_progress' && <span className="text-[9px] font-bold bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded uppercase">En cours</span>}
                                                    </div>
                ...
                <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-gray-200 dark:border-slate-700 h-fit shadow-sm">
                    ...
                    <div className="space-y-3">
                        {urgentTodos.length === 0 ? (
                            <p className="text-gray-400 dark:text-slate-500 text-sm italic">Rien d'urgent.</p>
                        ) : (
                            urgentTodos.map(t => (
                                <div 
                                    key={t.id} 
                                ...
                                >
                                    {/* NOUVEAU BOUTON COCHE COHÉRENT AVEC TODOLIST */}
