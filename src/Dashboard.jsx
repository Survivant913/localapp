...
export default function Dashboard({ data, updateData, setView }) {
    ...
    return (
        <div className="space-y-6 fade-in p-4 pb-24 md:pb-20 w-full bg-slate-50/10 dark:bg-slate-950/10 transition-all duration-300">
            {focusedProject && (
                <FocusProjectModal 
            ...
            {/* HEADER - DESIGN PREMIUM CENTRALISÉ SANS BONJOUR */}
            <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex-1 bg-white/70 dark:bg-slate-800/60 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-white dark:border-white/5 flex flex-col justify-between group relative overflow-hidden">
                    ...
                    <div className="flex flex-col items-center justify-center text-center mb-8 relative z-10">
                        <h2 className="text-3xl md:text-5xl font-black text-slate-800 dark:text-white mb-2 tracking-tighter italic uppercase">
                            Tableau de Bord
                        </h2>
                        <p className="text-blue-600 dark:text-blue-400 text-lg font-bold uppercase tracking-[0.2em]">{todayDate}</p>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-6 relative z-10">
                        <select 
                            ...
                            className="w-full md:w-auto px-6 py-3 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-2xl text-sm font-bold outline-none text-slate-800 dark:text-white shadow-sm ring-1 ring-slate-100 dark:ring-slate-700 transition-all"
                        >
                            <option value="total">Vue Stratégique</option>
                            {accounts.map(acc => <option key={acc.id} value={acc.id}>{acc.name}</option>)}
                        </select>
                        <div className="flex gap-4 w-full md:w-auto">
                            <button onClick={() => setView('budget')} className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl text-sm font-bold shadow-xl hover:scale-105 active:scale-95 transition-all"><Plus size={18}/> Dépense</button>
                            <button onClick={() => setView('todo')} className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-white dark:bg-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-2xl text-sm font-bold shadow-sm active:scale-95"><CheckSquare size={18} className="text-orange-500"/> Tâche</button>
                            <button onClick={() => setView('notes')} className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-white dark:bg-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-2xl text-sm font-bold shadow-sm active:scale-95"><StickyNote size={18} className="text-yellow-500"/> Note</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {/* CARTE SOLDE - OEIL ET LOGIQUE RÉTABLIS */}
                                    ...
                                    <div key={idx} className="flex items-center justify-between p-4 md:p-6 bg-slate-50/50 dark:bg-slate-800/40 rounded-[1.5rem] border border-slate-100 dark:border-slate-800 hover:bg-white dark:hover:bg-slate-800 transition-all duration-300 shadow-sm">
                                        <div className="flex items-center gap-5 overflow-hidden">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <div className="flex items-center gap-2 text-xs text-slate-400 font-bold uppercase tracking-tighter mt-1">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                        {pinnedNotes.map(n => (
                            <div key={n.id} className={`p-10 rounded-[2.5rem] border border-white/20 shadow-2xl ${n.color} text-slate-900 relative overflow-hidden cursor-pointer group hover:scale-[1.02] transition-all shadow-sm`} onClick={() => setView('notes')}>
                                ...
                                <h4 className="font-black text-xl mb-4 tracking-tighter">{n.title}</h4>
                    ...
                    <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-2xl relative overflow-hidden">
                        ...
                        <div className="space-y-4 relative z-10">
                            {urgentTodos.length === 0 ? (
                                <p className="text-slate-400 text-sm font-bold italic py-6 text-center opacity-50 uppercase tracking-widest">Feu éteint</p>
                            ) : (
                                urgentTodos.map(t => (
                                    <div key={t.id} className="group flex items-center gap-5 p-5 bg-white dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-700 hover:border-rose-500/30 transition-all cursor-pointer shadow-sm shadow-indigo-500/5" onClick={() => toggleTodo(t.id)}>
                                        <div className="w-7 h-7 rounded-xl border-2 border-slate-200 dark:border-slate-600 flex items-center justify-center group-hover:border-emerald-500 group-hover:bg-emerald-500 transition-all text-transparent group-hover:text-white shadow-inner">
