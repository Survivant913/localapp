...
import { 
  ...
  Highlighter, Quote, AlignRight, AlignJustify, X, Home, Palette
} from 'lucide-react';
...
export default function JournalManager({ data, updateData }) {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            // On récupère tout pour éviter les trous
    ...
    }, [activePageId]); // Retiré allPages des dépendances pour éviter refresh boucle
    ...
    // --- 5. CRÉATION (AVEC USER_ID RÉPARÉ) ---
    const createItem = async (type) => {
        // type = 'root' | 'folder' | 'page'
        let name = '';
    ...
    const deleteItem = async (id, type) => {
        ...
        try {
            if (type === 'page') {
                ...
                // Suppression en cascade simplifiée (Note: SQL cascade est mieux, mais ici on fait manuel)
    ...
    const ToolbarButton = ({ icon: Icon, cmd, val, active }) => (
        <button 
            onMouseDown={(e) => { e.preventDefault(); execCmd(cmd, val); }}
            className={`p-1.5 rounded transition-colors ${active ? 'bg-indigo-100 text-indigo-600' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 dark:text-slate-400'}`}
        >
            <Icon size={16}/>
        </button>
    );

    // --- IMPRESSION (FIX COULEURS) ---
    const handlePrint = () => {
        if (!activePageId) return;
    ...
    // --- FILTRAGE ---
    const rootFolders = allFolders.filter(f => !f.parent_id); // Ceux qui n'ont pas de parent
    ...
    return (
        <div className="flex h-full w-full bg-slate-50 dark:bg-slate-950 overflow-hidden">
                    ...
                    <>
                        <div className="h-14 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between px-4 sticky top-0 bg-white dark:bg-black z-20">
                            ...
                            <div className="flex items-center gap-1 overflow-x-auto no-scrollbar">
                                ...
                                <div className="flex gap-1 px-2 border-r border-slate-100 dark:border-slate-800">
                                ...
                                <div className="relative mx-2">
                                    ...
                                    {showColorPalette && (
                                        <div className="absolute top-full left-0 mt-2 flex gap-1 bg-white dark:bg-slate-800 border dark:border-slate-700 p-2 rounded-lg shadow-xl z-50 animate-in fade-in slide-in-from-top-2">
                                ...
                                <div className="flex gap-1 px-2 border-l border-slate-100 dark:border-slate-800">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="text-xs text-slate-400 font-mono w-24 text-right">
                                    {isSaving ? <span className="flex items-center justify-end gap-1 text-indigo-500"><Loader2 size={12} className="animate-spin"/> Save...</span> : <span className="flex items-center justify-end gap-1"><CheckSquare size={12}/> Prêt</span>}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto">
                            <div className="max-w-3xl mx-auto px-10 py-16 min-h-full">
                                ...
                                <input 
                                    ...
                                    className="w-full text-5xl font-black bg-transparent outline-none mb-10 text-slate-900 dark:text-white placeholder:text-slate-200 dark:placeholder:text-slate-800 leading-tight" 
                                    placeholder="Titre..."
                                />
                                <div 
                                    ref={editorRef} 
                                    ...
                                    onBlur={() => saveCurrentPage(true)} 
                                    className="prose dark:prose-invert max-w-none outline-none min-h-[50vh] text-lg leading-loose text-slate-600 dark:text-slate-300 empty:before:content-[attr(placeholder)] empty:before:text-slate-300" 
            ...
            <style>{`
                .prose blockquote { border-left: 4px solid #e2e8f0; padding-left: 1em; margin-left: 0; color: #64748b; font-style: italic; }
