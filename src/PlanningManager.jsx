...
export default function PlanningManager({ data, updateData }) {
    ...
    const handleSave = () => {
        ...
        // --- VERROU DE SÉCURITÉ FINAL ---
        const isActuallyRecurring = eventForm.recurrence || !!eventForm.recurrenceGroupId;
        const finalInvitedEmail = isActuallyRecurring ? '' : eventForm.invitedEmail;
    ...
    const applyUpdate = (targetId, startObj, endObj, formData, mode) => {
        ...
        if (mode === 'series' && formData.recurrenceGroupId) {
            updatedEvents = updatedEvents.map(ev => {
                if (ev.recurrence_group_id === formData.recurrenceGroupId) {
                    ...
                    if (evStart.getDay() !== startObj.getDay() && originalEvent) {
                        evStart = addDays(evStart, Math.round((startOfDay(startObj) - startOfDay(parseISO(originalEvent.start_time))) / 86400000));
                    }
                    let s = formData.isAllDay ? setMinutes(setHours(evStart, 0), 0) : setMinutes(setHours(evStart, getHours(startObj)), getMinutes(startObj));
                    let e = formData.isAllDay ? setMinutes(setHours(evStart, 23), 59) : addMinutes(s, formData.duration);
                    const upd = { ...ev, title: formData.title, color: formData.color, start_time: s.toISOString(), end_time: e.toISOString(), is_all_day: formData.isAllDay, invited_email: '', status: 'accepted' };
        ...
        updateData({ ...data, calendar_events: updatedEvents });
        eventsToSave.forEach(ev => { supabase.from('calendar_events').update({ title: ev.title, start_time: ev.start_time, end_time: ev.end_time, color: ev.color, is_all_day: ev.is_all_day, recurrence_group_id: ev.recurrence_group_id, invited_email: ev.invited_email, status: ev.status }).eq('id', ev.id).then(); });
    ...
    const performDelete = (evt, series) => {
        let updatedEvents = events.filter(e => series ? e.recurrence_group_id !== evt.recurrence_group_id : e.id !== evt.id);
        updateData({ ...data, calendar_events: updatedEvents }, series ? { table: 'calendar_events', filter: { column: 'recurrence_group_id', value: evt.recurrence_group_id } } : { table: 'calendar_events', id: evt.id });
        setSelectedEvent(null); setConfirmMode(null);
    };

    const onDragStart = (e, item) => { if (resizeRef.current || item.status === 'pending') { e.preventDefault(); return; } setDraggedItem({ type: 'event', data: item }); e.dataTransfer.effectAllowed = "move"; const img = new Image(); img.src = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7'; e.dataTransfer.setDragImage(img, 0, 0); };
    const onDragOverGrid = (e, dayIndex) => { if (draggedItem?.data.is_all_day) { setPreviewSlot(null); return; } e.preventDefault(); const rect = e.currentTarget.getBoundingClientRect(); const y = e.clientY - rect.top; let hour = Math.floor(y / 60); let snap = Math.round((y % 60) / 15) * 15; if (snap === 60) { snap = 0; hour++; } let dur = differenceInMinutes(parseISO(draggedItem.data.end_time), parseISO(draggedItem.data.start_time)); setPreviewSlot({ dayIndex, top: hour * 60 + snap, height: Math.max(30, dur), timeLabel: `${hour}:${snap.toString().padStart(2, '0')}` }); };
    const onDropGrid = (e, day) => { e.preventDefault(); setPreviewSlot(null); if (!draggedItem || draggedItem.data.is_all_day) return; const rect = e.currentTarget.getBoundingClientRect(); const y = e.clientY - rect.top; let h = Math.floor(y / 60); let m = Math.round((y % 60) / 15) * 15; if (m === 60) { m = 0; h++; } const start = setMinutes(setHours(day, h), m); const evt = draggedItem.data; const dur = differenceInMinutes(parseISO(evt.end_time), parseISO(evt.start_time)); const end = addMinutes(start, dur); if (evt.recurrence_group_id) { setEventForm({ ...eventForm, id: evt.id, title: evt.title, color: evt.color, recurrenceGroupId: evt.recurrence_group_id, duration: dur, date: format(start, 'yyyy-MM-dd'), startHour: h, startMin: m, recurrence: true }); setPendingUpdate({ newStart: start, newEnd: end, title: evt.title, color: evt.color, recurrenceGroupId: evt.recurrence_group_id, duration: dur, isAllDay: false }); setConfirmMode('ask_update'); setIsCreating(true); } else { updateData({ ...data, calendar_events: events.map(ev => ev.id === evt.id ? { ...ev, start_time: start.toISOString(), end_time: end.toISOString(), is_all_day: false } : ev) }, { table: 'calendar_events', id: evt.id }); } setDraggedItem(null); };
    const onDropAllDay = (e, day) => { e.preventDefault(); if (!draggedItem || !draggedItem.data.is_all_day) return; const evt = draggedItem.data; const s = setMinutes(setHours(day, 0), 0); const en = setMinutes(setHours(day, 23), 59); if (evt.recurrence_group_id) { setEventForm({ ...eventForm, id: evt.id, title: evt.title, color: evt.color, recurrenceGroupId: evt.recurrence_group_id, duration: 60, date: format(day, 'yyyy-MM-dd'), startHour: 9, startMin: 0, recurrence: true, isAllDay: true }); setPendingUpdate({ newStart: s, newEnd: en, title: evt.title, color: evt.color, recurrenceGroupId: evt.recurrence_group_id, duration: 60, isAllDay: true }); setConfirmMode('ask_update'); setIsCreating(true); } else { updateData({ ...data, calendar_events: events.map(ev => ev.id === evt.id ? { ...ev, start_time: s.toISOString(), end_time: en.toISOString(), is_all_day: true } : ev) }, { table: 'calendar_events', id: evt.id }); } setDraggedItem(null); };
                                ...
                                {weekDays.map((day, dayIndex) => {
                                    ...
                                    return (
                                        <div key={dayIndex} className="relative min-w-0 bg-white dark:bg-slate-900">
                                            ...
                                            <div className={`h-24 border-b-4 border-gray-200 dark:border-slate-800 bg-gray-50/30 dark:bg-slate-800/20 p-1 flex flex-col gap-1 overflow-y-auto custom-scrollbar transition-colors ${draggedItem ? 'hover:bg-blue-50/50 dark:hover:bg-blue-900/20' : ''}`} onDragOver={e => { if (draggedItem && !draggedItem.data.is_all_day) return; e.preventDefault(); }} onDrop={(e) => onDropAllDay(e, day)}>
                                                {allDayItems.map(item => {
                                                    ...
                                                    return (<div key={item.data.id} draggable={item.data.status !== 'pending'} onDragStart={(e) => onDragStart(e, item.data)} onClick={(e) => handleEventClick(e, item.data)} style={draggedItem?.data?.id === item.data.id ? { opacity: 0.5 } : {}} className={`text-[10px] font-bold px-2 py-1 rounded border border-l-4 truncate cursor-pointer hover:opacity-80 transition-all ${colorClass} ${item.data.status === 'pending' ? 'border-dashed opacity-70' : ''}`}>{item.data.status === 'pending' && '🔔 '}{item.data.title}</div>);
                                                })}
                                            </div>
                                        ...
                                        <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-700">
                                            <div className="flex items-center justify-between mb-2">
                                                <label className="flex items-center gap-2 cursor-pointer">
                                                    {/* GREFFE : ON VIDE L'EMAIL SI ON COCHE RÉCURRENCE */}
                            ...
                            <>
                                ...
                                {selectedEvent.data.status === 'pending' && selectedEvent.data.invited_email === data.profile?.email ? (
                                    <div className="space-y-6">
                                        <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-sm flex gap-3 text-blue-700 dark:text-blue-300"><Bell size={20}/><span>Invitation reçue. Accepter pour toute la série ?</span></div>
                                        <div className="grid gap-3">
                                            ...
                                            <button onClick={() => handleInvitation(selectedEvent.data, 'accepted', false)} className="w-full py-3 border border-emerald-600 text-emerald-600 rounded-xl font-bold flex items-center justify-center gap-2">Uniquement celui-là</button>
                                            <button onClick={() => handleInvitation(selectedEvent.data, 'declined', true)} className="w-full py-3 bg-red-50 text-red-600 rounded-xl font-bold flex items-center justify-center gap-2">Refuser tout</button>
                                        </div>
                                    </div>
                                ) : (
                                    <>
                                        ...
                                        {selectedEvent.data.invited_email && (
                                            <div className="mb-4 text-xs text-slate-500 flex flex-col gap-1">
                                                <div className="flex items-center gap-2"><UserPlus size={14}/>{selectedEvent.data.user_id === data.profile?.id ? (<span>Invité : <strong>{selectedEvent.data.invited_email}</strong></span>) : (<span>Organisateur : <strong>Collaborateur externe</strong></span>)}</div>
                                                <span className={`w-fit px-2 py-0.5 rounded-full text-[10px] font-bold ${selectedEvent.data.status === 'pending' ? 'bg-amber-100 text-amber-600' : selectedEvent.data.status === 'declined' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>{selectedEvent.data.status === 'pending' ? '⌛ En attente' : selectedEvent.data.status === 'declined' ? '❌ Refusé' : '✅ Confirmé'}</span>
                                            </div>
                                        )}
                                        <div className="flex gap-3">
                                            ...
                                            <button onClick={() => handleDeleteRequest(selectedEvent.data)} className="flex-1 py-3 bg-red-50 text-red-600 rounded-xl font-bold text-sm hover:bg-red-100 flex items-center justify-center gap-2 transition-colors"><Trash2 size={18}/> {selectedEvent.data.user_id === data.profile?.id ? 'Supprimer' : 'Retirer'}</button>
                                        </div>
                                    </>
                                )}
                            </>
