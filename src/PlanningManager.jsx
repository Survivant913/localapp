...
export default function PlanningManager({ data, updateData }) {
    ...
    const isItemAllDay = (item) => item?.data?.is_all_day === true;
    ...
    const handleInvitation = async (evt, newStatus, applyToSeries = false) => {
        ...
        if (applyToSeries && evt.recurrence_group_id) {
            idsToUpdate = events.filter(ev => ev.recurrence_group_id === evt.recurrence_group_id).map(ev => ev.id);
        }
        updatedEvents = events.map(ev => idsToUpdate.includes(ev.id) ? { ...ev, status: newStatus } : ev);
        updateData({ ...data, calendar_events: updatedEvents });
        await supabase.from('calendar_events').update({ status: newStatus }).in('id', idsToUpdate);
        setSelectedEvent(null); setConfirmMode(null);
    };

    const cleanPastEvents = async () => {
        const now = new Date();
        const activeEvents = events.filter(ev => !ev.end_time || parseISO(ev.end_time) > now);
        if (events.length === activeEvents.length) return alert("Agenda déjà propre !");
        if (window.confirm(`⚠️ SUPPRIMER ${events.length - activeEvents.length} ÉVÉNEMENTS PASSÉS ?`)) {
            updateData({ ...data, calendar_events: activeEvents });
            const ids = events.filter(ev => ev.end_time && parseISO(ev.end_time) <= now).map(ev => ev.id);
            if (ids.length > 0) await supabase.from('calendar_events').delete().in('id', ids);
        }
    };
    ...
    useEffect(() => {
        const handleMouseMove = (e) => {
            ...
            let newDuration = Math.max(15, Math.round((resizeRef.current.startDuration + deltaY) / 15) * 15);
        ...
        if (resizingEvent) { window.addEventListener('mousemove', handleMouseMove); window.addEventListener('mouseup', handleMouseUp); }
    ...
    const startResize = (e, evt) => {
        ...
        let dur = differenceInMinutes(parseISO(evt.end_time), parseISO(evt.start_time));
        const init = { id: evt.id, startY: e.clientY, startDuration: dur, currentDuration: dur, currentDurationTemp: dur, event: evt };
        setResizingEvent(init); resizeRef.current = init;
    };
    ...
    const finishResize = (evt, newDuration) => {
        ...
        if (evt.recurrence_group_id) {
            ...
            updateData({ ...data, calendar_events: events.map(ev => ev.id === evt.id ? { ...ev, end_time: newEnd.toISOString() } : ev) }, { table: 'calendar_events', id: evt.id });
        }
    };

    const handleEventClick = (e, itemData) => {
        e.stopPropagation();
        if (!ignoreNextClick.current) setSelectedEvent({ type: 'event', data: itemData });
    };

    const getLayoutForDay = (dayItems) => {
        const timed = dayItems.filter(i => !isItemAllDay(i)).sort((a, b) => parseISO(a.startStr) - parseISO(b.startStr));
        const columns = [];
        timed.forEach((item) => {
            ...
            for(let col of columns) {
                if (!col.some(ev => {
                    const eS = parseISO(ev.startStr); const eE = parseISO(ev.endStr);
                    let eD = resizingEvent && ev.data.id === resizingEvent.id ? resizingEvent.currentDuration : differenceInMinutes(eE, eS);
                    return (top < (getHours(eS) * 60 + getMinutes(eS)) + eD && top + duration > (getHours(eS) * 60 + getMinutes(eS)));
        ...
        const res = [];
        columns.forEach((col, idx) => col.forEach(item => res.push({ ...item, style: { top: `${item.top}px`, height: `${item.height}px`, width: `${100/columns.length}%`, left: `${(idx*100)/columns.length}%`, position: 'absolute' } })));
        return res;
    };
    ...
    const handleSave = () => {
        ...
        if (!eventForm.id) {
            ...
            const eventBase = { user_id: data.profile?.id, title: eventForm.title, color: eventForm.color, recurrence_group_id: groupId, recurrence_type: eventForm.recurrence ? 'weekly' : null, is_all_day: eventForm.isAllDay, invited_email: finalInvitedEmail, status: finalInvitedEmail ? 'pending' : 'accepted', organizer_email: data.profile?.email };
            let newEvts = [];
            if (eventForm.recurrence) {
                for (let i = 0; i < (parseInt(eventForm.recurrenceWeeks) || 1); i++) { newEvts.push({ ...eventBase, id: Date.now() + i, start_time: addWeeks(newStart, i).toISOString(), end_time: addWeeks(newEnd, i).toISOString() }); }
            } else { newEvts.push({ ...eventBase, id: Date.now(), start_time: newStart.toISOString(), end_time: newEnd.toISOString() }); }
            updateData({ ...data, calendar_events: [...events, ...newEvts] });
            supabase.from('calendar_events').insert(newEvts).then();
        } else {
            applyUpdate(eventForm.id, newStart, newEnd, { ...eventForm, invitedEmail: finalInvitedEmail }, eventForm.recurrenceGroupId ? 'ask' : 'single');
        }
        setIsCreating(false);
    };

    const applyUpdate = (targetId, startObj, endObj, formData, mode) => {
        let updated = [...events]; let toSave = [];
        const original = events.find(e => e.id === targetId);
        const resetStat = formData.invitedEmail && (formData.invitedEmail !== original?.invited_email);
        if (mode === 'series' && formData.recurrenceGroupId) {
            updated = updated.map(ev => {
                if (ev.recurrence_group_id === formData.recurrenceGroupId) {
                    ...
                    if (s.getDay() !== startObj.getDay()) s = addDays(s, Math.round((startOfDay(startObj) - startOfDay(parseISO(original.start_time))) / 86400000));
            ...
            updated = updated.map(ev => {
                if (ev.id === targetId) {
                    const upd = { ...ev, title: formData.title, color: formData.color, start_time: startObj.toISOString(), end_time: endObj.toISOString(), recurrence_group_id: null, is_all_day: formData.isAllDay, invited_email: formData.invitedEmail, status: resetStat ? 'pending' : ev.status };
                    toSave.push(upd); return upd;
                }
                return ev;
            });
        ...
        toSave.forEach(ev => supabase.from('calendar_events').update({ title: ev.title, start_time: ev.start_time, end_time: ev.end_time, color: ev.color, is_all_day: ev.is_all_day, recurrence_group_id: ev.recurrence_group_id, invited_email: ev.invited_email, status: ev.status, organizer_email: data.profile?.email }).eq('id', ev.id).then());
    ...
    const performDelete = (evt, series) => {
        const updated = events.filter(e => series ? e.recurrence_group_id !== evt.recurrence_group_id : e.id !== evt.id);
    ...
    const durOptions = useMemo(() => { const res = []; for (let m = 15; m <= 720; m += 15) { const h = Math.floor(m / 60); const min = m % 60; res.push(<option key={m} value={m}>{h > 0 ? `${h}h` : ''}{min > 0 ? ` ${min}` : h === 0 ? ' min' : ''}</option>); } return res; }, []);
        ...
        <div className="fade-in flex flex-col md:flex-row h-full w-full overflow-hidden bg-gray-50 dark:bg-slate-950 font-sans">
                ...
                <div className="flex-1 bg-white dark:bg-slate-900 rounded-3xl border border-gray-200 dark:border-slate-800 shadow-xl flex flex-col overflow-hidden relative">
                    <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur z-30 sticky top-0">
                        <div className="flex items-center gap-4">
                            <h2 className="text-xl font-bold text-slate-800 dark:text-white capitalize flex items-center gap-2"><Calendar className="text-blue-500" size={24}/>{format(currentWeekStart, 'MMMM yyyy', { locale: fr })}</h2>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button onClick={() => openCreateModal()} className="py-2 px-4 bg-slate-900 dark:bg-white text-white dark:text-black rounded-xl font-bold text-xs flex items-center gap-2"><Plus size={16}/> Planifier</button>
                        </div>
                    </div>

                    <div className="flex-1 overflow-y-auto custom-scrollbar select-none relative h-full">
                        <div className="flex w-full h-full min-h-[1440px]">
                            <div className="w-16 flex-shrink-0 border-r border-gray-200 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-900 sticky left-0 z-20">
                                ...
                                <div className="h-24 border-b-4 border-gray-200 dark:border-slate-800 flex flex-col justify-center items-center text-[10px] font-bold text-slate-400 bg-white dark:bg-slate-900"><span>ALL</span><span>DAY</span></div>
                            ...
                            <div className="flex-1 grid grid-cols-7 divide-x divide-gray-200 dark:divide-slate-800">
                                {weekDays.map((day, dayIndex) => {
                                    ...
                                    const raw = events.filter(e => isSameDay(parseISO(e.start_time), day)).map(e => ({ data: e, startStr: e.start_time, endStr: e.end_time }));
                                    const layout = getLayoutForDay(raw);
                                    return (
                                        <div key={dayIndex} className="relative min-w-0 bg-white dark:bg-slate-900">
                                            <div className={`h-14 flex flex-col items-center justify-center border-b border-gray-200 dark:border-slate-800 sticky top-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur z-30 ${isToday ? 'bg-blue-50/80 dark:bg-blue-900/20' : ''}`}>
                                                <span className={`text-[10px] font-bold uppercase ${isToday ? 'text-blue-600' : 'text-slate-400'}`}>{format(day, 'EEE', { locale: fr })}</span>
                                            ...
                                            <div className="h-24 border-b-4 border-gray-200 dark:border-slate-800 bg-gray-50/30 dark:bg-slate-800/20 p-1 flex flex-col gap-1 overflow-y-auto custom-scrollbar" onDragOver={e => { if (draggedItem && !draggedItem.data.is_all_day) return; e.preventDefault(); }} onDrop={e => { e.preventDefault(); if (draggedItem?.data.is_all_day) updateData({ ...data, calendar_events: events.map(ev => ev.id === draggedItem.data.id ? { ...ev, start_time: setMinutes(setHours(day, 0), 0).toISOString(), end_time: setMinutes(setHours(day, 23), 59).toISOString() } : ev) }, { table: 'calendar_events', id: draggedItem.data.id }); setDraggedItem(null); }}>
                                                {raw.filter(i => isItemAllDay(i)).map(item => (<div key={item.data.id} draggable={item.data.status !== 'pending'} onDragStart={(e) => onDragStart(e, item.data)} onClick={(e) => handleEventClick(e, item.data)} style={draggedItem?.data?.id === item.data.id ? { opacity: 0.5 } : {}} className={`text-[10px] font-bold px-2 py-1 rounded border border-l-4 truncate cursor-pointer ${item.data.color === 'green' ? 'bg-emerald-100 text-emerald-800 border-l-emerald-500' : item.data.color === 'gray' ? 'bg-slate-100 text-slate-700 border-l-slate-500' : 'bg-blue-100 text-blue-800 border-l-blue-500'} ${item.data.status === 'pending' ? 'border-dashed opacity-70' : ''}`}>{item.data.status === 'pending' && '🔔 '}{item.data.title}</div>))}
                                            </div>
                                            <div className="relative h-[1440px]" onDragOver={(e) => onDragOverGrid(e, dayIndex)} onDrop={(e) => onDropGrid(e, day)}>
                                                ...
                                                {isToday && (<div className="absolute w-full border-t-2 border-red-500 z-10 pointer-events-none" style={{ top: `${(getHours(new Date()) * 60 + getMinutes(new Date()))}px` }}><div className="w-2 h-2 bg-red-500 rounded-full -ml-1"></div></div>)}
                                                ...
                                                {layout.map((item) => {
                                                    ...
                                                    const colorClass = dataItem.color === 'green' ? 'bg-white border-l-4 border-l-emerald-500 text-emerald-900 dark:bg-slate-800 dark:text-emerald-100' : dataItem.color === 'gray' ? 'bg-white border-l-4 border-l-slate-500 text-slate-700 dark:bg-slate-800 dark:text-slate-300' : 'bg-white border-l-4 border-l-blue-600 text-blue-900 dark:bg-slate-800 dark:text-blue-100';
                                                    return (
                                                        <div key={item.data.id} style={{ ...item.style, opacity: draggedItem?.data?.id === item.data.id ? 0.5 : (isPending ? 0.6 : 1) }} draggable={!resizeRef.current && !isPending} onDragStart={(e) => onDragStart(e, dataItem)} onClick={(e) => handleEventClick(e, dataItem)} className={`absolute rounded-r-lg rounded-l-sm p-2 text-xs cursor-pointer shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden flex flex-col group/item select-none ${colorClass} ${isPending ? 'border-dashed' : ''}`}>
                                                            <span className="font-bold truncate text-[11px] flex items-center gap-1">{isPending && <Bell size={10} className="text-blue-500"/>}{item.data.title}</span>
                                                            <div className="flex items-center gap-1 mt-auto pt-1 opacity-80 text-[10px] font-semibold">{format(parseISO(item.startStr), 'HH:mm')}{!!item.data.recurrence_group_id && <Repeat size={10} />}</div>
                                                            {!isPending && (<div className="absolute bottom-0 left-0 w-full h-3 cursor-s-resize hover:bg-black/5 transition-colors z-50 flex items-center justify-center opacity-0 group-hover/item:opacity-100" onMouseDown={(e) => startResize(e, item.data)}><div className="w-8 h-1 bg-black/20 dark:bg-white/30 rounded-full"></div></div>)}
                                                        </div>
                                                    );
                                                })}
                                            </div>
            ...
            {isCreating && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-2xl w-full max-w-sm border border-slate-200 dark:border-slate-700">
                        {confirmMode === 'ask_update' ? (
                            <div className="text-center space-y-4">
                                <div className="w-12 h-12 bg-blue-100 text-blue-500 rounded-full flex items-center justify-center mx-auto"><Repeat size={24}/></div>
                                <h3 className="font-bold text-lg dark:text-white">Modifier la récurrence ?</h3>
                                <div className="grid gap-3">
                                    <button onClick={() => applyUpdate(eventForm.id, pendingUpdate.newStart, pendingUpdate.newEnd, pendingUpdate, 'single')} className="w-full py-3 border border-slate-200 rounded-xl font-bold text-sm dark:text-white">Juste celui-là</button>
                                    <button onClick={() => applyUpdate(eventForm.id, pendingUpdate.newStart, pendingUpdate.newEnd, pendingUpdate, 'series')} className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold text-sm">Toute la série</button>
                                    <button onClick={() => setIsCreating(false)} className="text-xs text-slate-400 mt-2">Annuler</button>
                                </div>
                            </div>
                        ) : (
                            <>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex items-center gap-2 py-2">
                                        <input type="checkbox" id="allDay" checked={eventForm.isAllDay} onChange={e => setEventForm({...eventForm, isAllDay: e.target.checked})} className="w-5 h-5 text-blue-600 rounded"/>
                                        <label htmlFor="allDay" className="text-sm font-bold text-slate-700 dark:text-slate-300">Toute la journée</label>
                                    </div>
                                    {!eventForm.isAllDay && (
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="flex gap-1"><select value={eventForm.startHour} onChange={e => setEventForm({...eventForm, startHour: parseInt(e.target.value)})} className="w-full p-2 bg-slate-50 dark:bg-slate-800 border rounded-lg dark:text-white outline-none">{Array.from({length:24}).map((_,i)=><option key={i} value={i}>{i}h</option>)}</select><select value={eventForm.startMin} onChange={e => setEventForm({...eventForm, startMin: parseInt(e.target.value)})} className="w-full p-2 bg-slate-50 dark:bg-slate-800 border rounded-lg dark:text-white outline-none"><option value={0}>00</option><option value={15}>15</option><option value={30}>30</option><option value={45}>45</option></select></div>
                                            <select value={eventForm.duration} onChange={e => setEventForm({...eventForm, duration: parseInt(e.target.value)})} className="w-full p-2 bg-slate-50 dark:bg-slate-800 border rounded-lg dark:text-white outline-none">{durOptions}</select>
                                        </div>
                                    )}
                                    {!eventForm.id && (
                                        <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-700">
                                            <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={eventForm.recurrence} onChange={e => setEventForm({...eventForm, recurrence: e.target.checked, invitedEmail: e.target.checked ? '' : eventForm.invitedEmail})} className="w-4 h-4 rounded text-blue-600"/><span className="text-sm font-bold text-slate-700 dark:text-slate-300">Répéter (Hebdo)</span></label>
                                            {eventForm.recurrence && (<div className="flex items-center gap-2 mt-2"><span className="text-xs text-slate-500">Pendant</span><input type="number" min="1" max="52" value={eventForm.recurrenceWeeks} onChange={e => setEventForm({...eventForm, recurrenceWeeks: e.target.value})} className="w-16 p-1 bg-white dark:bg-slate-800 border rounded text-sm"/><span className="text-xs text-slate-500">semaines</span></div>)}
                                        </div>
                                    )}
                                    <div className="flex gap-3 pt-2">{['blue', 'green', 'gray'].map(c => (<button key={c} onClick={() => setEventForm({...eventForm, color: c})} className={`flex-1 h-8 rounded-lg border-2 ${eventForm.color === c ? 'border-slate-800 dark:border-white' : 'border-transparent opacity-40'} ${c === 'blue' ? 'bg-blue-500' : c === 'green' ? 'bg-emerald-500' : 'bg-slate-500'}`}></button>))}</div>
                                    <div className="flex gap-3 pt-4"><button onClick={() => setIsCreating(false)} className="flex-1 py-3 text-slate-600 hover:bg-slate-100 rounded-xl font-bold">Annuler</button><button onClick={handleSave} className="flex-1 py-3 bg-slate-900 dark:bg-white text-white dark:text-black rounded-xl font-bold">Sauver</button></div>
                                </div>
                            </>
                        )}
                    </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-2xl w-full max-w-sm border border-slate-200 dark:border-slate-700">
                        {confirmMode === 'ask_cancel_series' ? (
                            <div className="text-center space-y-4">
                                <div className="w-12 h-12 bg-orange-100 text-orange-500 rounded-full flex items-center justify-center mx-auto"><Ban size={24}/></div>
                                <h3 className="font-bold text-lg dark:text-white">Retirer la série ?</h3>
                                <div className="grid gap-3">
                                    <button onClick={() => handleInvitation(selectedEvent.data, 'declined', false)} className="w-full py-3 border rounded-xl font-bold text-sm dark:text-white">Juste celui-là</button>
                                    <button onClick={() => handleInvitation(selectedEvent.data, 'declined', true)} className="w-full py-3 bg-orange-600 text-white rounded-xl font-bold text-sm">Toute la série</button>
                                    <button onClick={() => setConfirmMode(null)} className="text-xs text-slate-400">Annuler</button>
                                </div>
                            </div>
                        ) : confirmMode === 'ask_delete' ? (
                            <div className="text-center space-y-4">
                                <div className="w-12 h-12 bg-red-100 text-red-500 rounded-full flex items-center justify-center mx-auto"><AlertTriangle size={24}/></div>
                                <h3 className="font-bold text-lg dark:text-white">Supprimer la série ?</h3>
                                <div className="grid gap-3">
                                    <button onClick={() => performDelete(selectedEvent.data, false)} className="w-full py-3 border border-slate-200 rounded-xl font-bold text-sm dark:text-white">Juste celui-là</button>
                                    <button onClick={() => performDelete(selectedEvent.data, true)} className="w-full py-3 bg-red-500 text-white rounded-xl font-bold text-sm">Toute la série</button>
                                    <button onClick={() => setConfirmMode(null)} className="text-xs text-slate-400">Annuler</button>
                                </div>
                            </div>
                        ) : (
                            <>
                                ...
                                {/* GREFFE LOGIQUE D'APPROBATION COHÉRENTE */}
                                {selectedEvent.data.status === 'pending' && selectedEvent.data.invited_email?.toLowerCase().includes(data.profile?.email?.toLowerCase()) ? (
                                    <div className="space-y-6">
                                        <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-sm flex gap-3 text-blue-700 dark:text-blue-300">
                                            ...
                                            <span>
                                                ...
                                                {selectedEvent.data.recurrence_group_id ? " Accepter pour toute la série ?" : " Souhaitez-vous accepter cet événement ?"}
                                            </span>
                                        </div>
                                        <div className="grid gap-3">
                                    ...
                                    <>
                                        <div className="mb-6 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-800 flex gap-4 items-center"><div className="p-3 bg-white dark:bg-slate-800 rounded-lg shadow-sm text-blue-600"><Clock size={24}/></div><div><p className="text-xs font-bold text-slate-400 uppercase">Horaire</p><p className="text-sm font-medium text-slate-700 dark:text-slate-300">{selectedEvent.data.is_all_day ? "Toute la journée" : format(parseISO(selectedEvent.data.start_time), 'EEEE d MMMM HH:mm', { locale: fr })}</p></div></div>
                                        ...
                                        <div className="flex gap-3">
                                            <button onClick={() => openEditModal(selectedEvent.data)} className="flex-1 py-3 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-bold flex items-center justify-center gap-2"><Pencil size={16}/> Modifier</button>
                                            <button onClick={() => handleDeleteRequest(selectedEvent.data)} className="flex-1 py-3 bg-red-50 text-red-600 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors"><Trash2 size={18}/> {selectedEvent.data.user_id === data.profile?.id ? 'Supprimer' : 'Retirer'}</button>
                                        </div>
                                    </>
                                )}
                            </>
