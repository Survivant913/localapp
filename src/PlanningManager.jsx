...
import { 
  ...
  setHours, setMinutes, addMinutes, differenceInMinutes, startOfDay, parse
} from 'date-fns';
...
export default function PlanningManager({ data, updateData }) {
    ...
    // Pour la suppression récurrente
    const [deleteMode, setDeleteMode] = useState(null); // 'single' | 'series' | null
    ...
    const [newEvent, setNewEvent] = useState({ 
        title: '', 
        date: format(new Date(), 'yyyy-MM-dd'), // Vrai date picker
        startHour: 9, 
        startMin: 0,
        duration: 60, 
        type: 'event', 
        recurrence: false, 
        recurrenceWeeks: 12, // Durée personnalisable
        color: 'blue' 
    });
    ...
    const getLayoutForDay = (dayItems) => {
        ...
        sorted.forEach((item) => {
            ...
            const top = Math.max(0, startMin - (6 * 60)); // Offset 6h du mat
    ...
    const saveEvent = () => {
        ...
        // Construction de la date exacte à partir du picker
        const baseDate = parse(newEvent.date, 'yyyy-MM-dd', new Date());
        const startDate = setMinutes(setHours(baseDate, newEvent.startHour), newEvent.startMin);
        const endDate = addMinutes(startDate, newEvent.duration);

        // ID de groupe pour la récurrence
        const groupId = newEvent.recurrence ? Date.now().toString() : null;

        const eventBase = {
            user_id: data.profile?.id,
            title: newEvent.title,
            color: newEvent.color,
            recurrence_group_id: groupId, // Lien entre les événements
            recurrence_type: newEvent.recurrence ? 'weekly' : null
        };

        let newEventsList = [];
        if (newEvent.recurrence) {
            const weeks = parseInt(newEvent.recurrenceWeeks) || 1;
            ...
            newEventsList.push({ ...eventBase, id: Date.now(), start_time: startDate.toISOString(), end_time: endDate.toISOString() });
        }

        updateData({ ...data, calendar_events: [...events, ...newEventsList] });
        setIsCreating(false);
    };

    const handleDeleteRequest = (evt) => {
        if (evt.recurrence_group_id) {
            // C'est une série -> Demander confirmation spéciale
            setDeleteMode('ask');
        } else {
            if(window.confirm("Supprimer cet événement ?")) {
                performDelete(evt, false);
            }
        }
    };

    const performDelete = (evt, deleteAllSeries) => {
        let updatedEvents = events;
        if (deleteAllSeries && evt.recurrence_group_id) {
            updatedEvents = events.filter(e => e.recurrence_group_id !== evt.recurrence_group_id);
        } else {
            updatedEvents = events.filter(e => e.id !== evt.id);
        }
        updateData({ ...data, calendar_events: updatedEvents }, { table: 'calendar_events', id: deleteAllSeries ? null : evt.id }); // Note: Supabase delete logic might need loop for bulk delete but local state is OK
        setSelectedEvent(null);
        setDeleteMode(null);
    };

    const scheduleTodo = (todo, date, hour) => {
    ...
    const onDragOver = (e, dayIndex) => {
        ...
        const minutesFromStart = e.clientY - rect.top; // 1px = 1min
        
        // Snap 15min
        let rawMinutes = Math.floor(minutesFromStart % 60);
        let snappedMinutes = Math.round(rawMinutes / 15) * 15;
        let hour = Math.floor(minutesFromStart / 60) + 6; // Start at 6h
        ...
        const top = (hour - 6) * 60 + snappedMinutes;
        
        setPreviewSlot({
            dayIndex, top, height: Math.max(30, duration),
    ...
    const onDrop = (e, day) => {
        ...
        if (draggedItem.type === 'todo') {
            ...
            // Si c'est une série, on détache cet événement de la série lors du déplacement (comportement standard)
            // Ou on le déplace juste lui.
    ...
    // HEURES : De 6h à 00h (donc jusqu'à 24h exclu, affichage 23h)
            ...
            <div className="w-full md:w-80 border-r border-slate-200 dark:border-slate-800 flex flex-col bg-slate-50 dark:bg-slate-900 z-20 shadow-xl shadow-slate-200/50 dark:shadow-none">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                    {backlogTodos.map(todo => (
                        <div 
                            key={todo.id} draggable onDragStart={(e) => onDragStart(e, todo, 'todo')}
                ...
                <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
                    <button onClick={() => { 
                        ...
                        setIsCreating(true); 
                    }} className="w-full py-3.5 bg-slate-900 dark:bg-white text-white dark:text-black rounded-xl font-bold text-sm hover:shadow-lg hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
                            ...
                            {weekDays.map((day, dayIndex) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {/* Hauteur ajustée pour 18h + 1h (00h) = 19h * 60px = 1140px */}
                                        <div className="relative h-[1140px]">
                                            {/* Lignes Heures */}
                                            {Array.from({length: 19}).map((_, i) => (
                                                 <div key={i} className="absolute w-full border-t border-slate-50 dark:border-slate-800/40 h-[60px]" style={{ top: `${i*60}px` }}></div>
                                            ))}
            ...
            {isCreating && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-2xl w-full max-w-sm animate-in zoom-in-95 border border-slate-200 dark:border-slate-700">
                        <h3 className="text-xl font-bold mb-6 text-slate-800 dark:text-white font-serif">Planifier</h3>
                        <div className="space-y-4">
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Titre</label>
                                <input autoFocus type="text" value={newEvent.title} onChange={e => setNewEvent({...newEvent, title: e.target.value})} className="w-full mt-1.5 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 outline-none focus:border-blue-500 dark:text-white" placeholder="Titre..." />
                            </div>
                            
                            {/* DATE EXACTE */}
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Date</label>
                                <input type="date" value={newEvent.date} onChange={e => setNewEvent({...newEvent, date: e.target.value})} className="w-full mt-1.5 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-sm dark:text-white outline-none"/>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select value={newEvent.startHour} onChange={e => setNewEvent({...newEvent, startHour: parseInt(e.target.value)})} className="w-full mt-1.5 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-sm dark:text-white outline-none">
                                        {Array.from({length: 19}).map((_, i) => <option key={i} value={i+6}>{i+6}:00</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Minutes</label>
                                    <select value={newEvent.startMin} onChange={e => setNewEvent({...newEvent, startMin: parseInt(e.target.value)})} className="w-full mt-1.5 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-sm dark:text-white outline-none">
                            ...
                            <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-700">
                                <div className="flex items-center justify-between mb-2">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" checked={newEvent.recurrence} onChange={e => setNewEvent({...newEvent, recurrence: e.target.checked})} className="w-4 h-4 rounded text-blue-600"/>
                                        <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Répéter (Hebdo)</span>
                                    </label>
                                </div>
                                {newEvent.recurrence && (
                                    <div className="flex items-center gap-2 mt-2 animate-in slide-in-from-top-2">
                                        <span className="text-xs text-slate-500">Pendant</span>
                                        <input type="number" min="1" max="52" value={newEvent.recurrenceWeeks} onChange={e => setNewEvent({...newEvent, recurrenceWeeks: e.target.value})} className="w-16 p-1 text-center bg-white dark:bg-slate-800 border rounded text-sm"/>
                            ...
                            <div className="flex gap-3 pt-2">
                                {['blue', 'green', 'gray'].map(c => (
                                    <button key={c} onClick={() => setNewEvent({...newEvent, color: c})} className={`flex-1 h-8 rounded-lg border-2 transition-all ${newEvent.color === c ? 'border-slate-800 dark:border-white opacity-100' : 'border-transparent opacity-40'} ${c === 'blue' ? 'bg-blue-500' : c === 'green' ? 'bg-emerald-500' : 'bg-slate-500'}`}></button>
                                ))}
                            </div>
                            
                            <div className="flex gap-3 pt-4">
                                <button onClick={() => setIsCreating(false)} className="flex-1 py-3 text-slate-600 hover:bg-slate-100 rounded-xl font-bold text-sm">Annuler</button>
                                <button onClick={saveEvent} className="flex-1 py-3 bg-slate-900 dark:bg-white text-white dark:text-black rounded-xl font-bold text-sm">Créer</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {selectedEvent && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-2xl w-full max-w-sm animate-in zoom-in-95 border border-slate-200 dark:border-slate-700">
                        ...
                        {/* MODE CONFIRMATION SUPPRESSION SÉRIE */}
                        {deleteMode === 'ask' ? (
                            <div className="text-center space-y-4">
                                ...
                                <h3 className="font-bold text-lg dark:text-white">Supprimer la récurrence ?</h3>
                                <p className="text-sm text-slate-500">Cet événement fait partie d'une série.</p>
                                <div className="grid gap-3">
                                    ...
                                    <button onClick={() => setDeleteMode(null)} className="text-xs text-slate-400 hover:underline mt-2">Annuler</button>
                                </div>
                            </div>
                        ) : (
                            <>
                                <div className="flex justify-between items-start mb-6">
                                    ...
                                    <button onClick={() => { setSelectedEvent(null); setDeleteMode(null); }} className="p-1 hover:bg-slate-100 rounded-full"><X size={20} className="text-slate-400"/></button>
                                </div>
                                ...
                                <div className="flex gap-3">
                                    {selectedEvent.type === 'todo' ? (
                                        <button onClick={() => unscheduleTodo(selectedEvent.data)} className="flex-1 py-3 border-2 border-orange-100 text-orange-600 rounded-xl font-bold text-sm hover:bg-orange-50 transition-colors">Retirer</button>
                                    ) : (
